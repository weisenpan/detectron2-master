...
def do_test(cfg, model, iteration):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        if comm.is_main_process():
            ...
            if not os.path.exists('inference4.csv'):
                with open('inference4.csv','w') as f:
            ...
            with open('inference4.csv','a') as f:
                for task, res in results_i.items():
                    important_res = [(k, v) for k, v in res.items() if "-" not in k]
                    f.write(localtime + ",")
                    f.write(task + ",")
...
if __name__ == "__main__":
    if os.path.exists('inference4.csv'):
        print("Old inference4.csv Exists! Delete Or Move It, And Try Again!")
        sys.exit(0)
    args = default_argument_parser().parse_args()
    args.config_file = "configs/COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_1x.yaml"
    args.num_gpus = 4
    args.opts = ['SOLVER.IMS_PER_BATCH', '8', 'SOLVER.BASE_LR', '0.005', 'TEST.EVAL_PERIOD', '1000', 'SEED', '123', 'OUTPUT_DIR', 'train4']
